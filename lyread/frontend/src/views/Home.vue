<template>
  <div class="home">
    ...
    <!-- 次屏转化：看完产品预览后立刻 register-first，对齐 Pricing/Trending -->
    <section class="home-mid-cta" aria-label="开始创作">
      ...
      <div class="mid-actions">
        <router-link
          ...
          class="btn primary"
          :to="demoCreateLink"
          @click="trackEvent('home_mid_register', { category: 'funnel', label: 'mid_cta' })"
        >免费注册开写 →</router-link>
        <router-link
          v-else
          class="btn primary"
          :to="{ path: '/workspace', query: { mode: 'new' } }"
          @click="trackEvent('home_mid_workspace', { category: 'funnel', label: 'mid_cta' })"
        >进入创作台 →</router-link>
        <router-link
          class="btn"
          to="/story"
          @click="trackEvent('home_mid_story', { category: 'funnel', label: 'mid_cta' })"
        >试试短故事</router-link>
        <router-link
          class="btn"
          to="/trending"
          @click="trackEvent('home_mid_trending', { category: 'funnel', label: 'mid_cta' })"
        >先看看案例</router-link>
      </div>
    </section>
...
.home-mid-cta {
  ...
  border: 1px solid rgba(218, 230, 245, 0.95);
  border-radius: 12px;
}
.home-mid-cta .mid-kicker {
  margin: 0 0 14px;
  font-size: 14px;
  color: #4a90d9;
  font-weight: 600;
}
.home-mid-cta .mid-actions {
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
...
@media (max-width: 768px) {
  ...
  .home-mid-cta .btn { width: 100%; text-align: center; }
  .data-item h3 {
    font-size: 30px;
  }
  .data-item p {

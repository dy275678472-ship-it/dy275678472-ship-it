...
def _unique_pad_beats(case: dict) -> list:
    ...
    banks = {
        ...
        "soninlaw": [
            ...
            f"越急着私下了结，越要留下第三人。",
            f"夜深人静。{name}给自己定序：止损、核验、公开。顺序错了，赢面也会变把柄。",
    ...
    default = [
        ...
        # 禁止跨案同句「未回消息/三行计划」；改绑书名+钩子的可核验备忘
        f"《{title}》备忘扉页——{name}只推进「{hook}」里今晚能核验的一步："
        f"人到场、物到手、时戳在；缺一则停，不拿空话凑篇幅。",
    ]
...
def pad_excerpt(text: str, case: dict, min_chars: int = 2000) -> str:
    ...
    genre_scene = {
        ...
        "romance": [
            ...
            f"他给关系排优先级：澄清、边界、再甜宠。顺序错了，甜也会涩。",
        ],
        "suspense": [
            ...
            f"他给侦办排优先级：时间线、物证、再动机。顺序错了，抓错人。",
        ],
        "history": [
            ...
            f"他给改史排优先级：粮草、信物、再声名。顺序错了，胜仗也是笑话。",
        ],
        "apocalypse": [
            ...
            f"他给末世排优先级：门、水、药。顺序错了，队伍会在恐慌里散。",
        ],
        "palace": [
            ...
            f"他给宫斗排优先级：避锋、存证、换援。顺序错了，翻盘变送人头。",
        ],
        "soninlaw": [
            ...
            f"他给翻身排优先级：止损、核验、公开。顺序错了，赢面变把柄。",
        ],
        "baby": [
            ...
            f"他给护短排优先级：接人、查校、留证。顺序错了，面子会压到小孩。",
        ],
        "horror": [
            ...
            f"他给逃生排优先级：出口、同伴、再取证。顺序错了，胆子也没用。",
        ],
        "scifi": [
            ...
            f"他给航程排优先级：隔离、采样、上报。顺序错了，舱压也会骗人。",
        ],
        "campus": [
            ...
            f"他给校争排优先级：存证、当面、找老师。顺序错了，少年气会烧到自己。",
        ],
    }
